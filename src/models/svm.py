...
class SVMSoftMargin(BaseModel):
    ...
    def train(self, X: np.ndarray, y: np.ndarray):
        ...
        P = cvxopt.matrix(K.T.dot(K)) # P has shape n*n
        q = cvxopt.matrix(-1 * np.ones(n_samples)) # q has shape n*1
        G = cvxopt.matrix(np.concatenate((-1*np.identity(n_samples),
                                          np.identity(n_samples)), axis=0))
        h = cvxopt.matrix(np.concatenate((np.zeros(n_samples),
                                          self.C*np.ones(n_samples)), axis=0))
        A = cvxopt.matrix(1.0 * y, (1, n_samples))
        b = cvxopt.matrix(0.0)
        # solve quadratic programming
        cvxopt.solvers.options['show_progress'] = False
